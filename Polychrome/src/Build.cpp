...
namespace Polychrome
{
	...
	bool Build::BuildMonoAssembly(const std::string& path, const std::string& name)
	{
		...
		int success = system((".\\mono\\bin\\mcs.bat -debug -target:library -nostdlib -out:\"" + std::filesystem::absolute(path).string() + "\\bin\\" + name + ".dll\" -r:Chroma.Mono.dll" + monoLibStr + " -recurse:\"" + std::filesystem::absolute(path).string() + "\\**.cs\" 2> " + logFile + " 1>&2").c_str());
