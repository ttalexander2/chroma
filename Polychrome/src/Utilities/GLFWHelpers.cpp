...
bool Polychrome::glfwSetWindowCenter(GLFWwindow* window)
{
    if (!window)
        return false;
    ...
    for (int j = 0; j < monitor_count; ++j)
    {
        ...
        int minX = glm::max(mx, px);
        int minY = glm::max(my, py);

        int maxX = glm::min(mx + mode->width, px + sx);
        int maxY = glm::min(my + mode->height, py + sy);

        // Calculate area of the intersection
        int area = glm::max(maxX - minX, 0) * glm::max(maxY - minY, 0);
