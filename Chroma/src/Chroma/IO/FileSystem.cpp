...
namespace Chroma
{
	...
	void FileSystem::Mount(const std::string &path)
	{
		...
		int result = PHYSFS_mount(path.c_str(), nullptr, 1);
		if (result == 0)
	...
	{
		return static_cast<FileSystem::Error>(PHYSFS_getLastErrorCode());
	}
	...
	std::string FileSystem::GetErrorByCode(Error errorCode)
	{
		return PHYSFS_getErrorByCode(static_cast<PHYSFS_ErrorCode>(errorCode));
	}
	...
	FileSystem::FileInfo FileSystem::GetFileInfo(const std::string &path)
	{
		...
		out.Type = static_cast<FileSystem::FileType>(statbuf.filetype);
	...
	FileSystem::FileType FileSystem::GetFileType(const std::string &filePath)
	{
		...
		return static_cast<FileSystem::FileType>(statbuf.filetype);
	}
	...
	File FileSystem::Open(const std::string &filePath, FileMode mode)
	{
