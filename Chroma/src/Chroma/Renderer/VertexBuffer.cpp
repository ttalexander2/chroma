...
namespace Chroma
{
	Ref<VertexBuffer> VertexBuffer::Create(uint32_t size, VertexBufferUsage usage)
	{
		...
		{
			case RendererAPI::API::None: CHROMA_CORE_ASSERT(false, "RendererAPI::API::None not supported!");
				return nullptr;
			case RendererAPI::API::OpenGL:
				return std::make_shared<OpenGLVertexBuffer>(size, usage);
			case RendererAPI::API::Vulkan:
				return std::make_shared<VulkanVertexBuffer>(size, usage);
		}
	...
	Ref<VertexBuffer> VertexBuffer::Create(float *vertices, uint32_t size, VertexBufferUsage usage)
	{
		switch (RendererAPI::GetAPI())
		{
			...
			case RendererAPI::API::Vulkan:
				return std::make_shared<VulkanVertexBuffer>(vertices, size, usage);
		}
