...
namespace Chroma
{
	...
	void ParticleSystem::Init()
	{
		for (auto& e : m_Scene->Registry.view<ParticleEmitter>())
		{
			...
			if (emitter.PreWarm)
			{
				...
				for (int i = 0; i < (int)glm::round(60 * emitter.PreWarmSeconds); i++)
				{
	...
	void ParticleSystem::UpdateEmitter(ParticleEmitter& emitter, glm::vec2 position,  Time delta)
	{
		...
		for (Particle& p : emitter.Particles)
		{
			...
			p.Position += ((glm::vec2(glm::cos(glm::radians(p.Angle)), glm::sin(glm::radians(p.Angle))) + glm::vec2(0, -emitter.Gravity)) * p.Speed + ((emitter.random() - 0.5f) * 2 * emitter.GravityVariance)) * (float)delta;
		}
	}
	...
	void ParticleSystem::Draw(Time delta)
	{
		for (auto& e : m_Scene->Registry.view<ParticleEmitter>())
		{
			...
			for (Particle& p : emitter.Particles)
			{
				...
				Math::vec4 color = glm::lerp(emitter.EndColor, emitter.StartColor, p.Life / p.TotalLife);
