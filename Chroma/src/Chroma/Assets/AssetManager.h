...
namespace Chroma
{
	...
	{
		static_assert(std::is_base_of_v<Asset, T>, "Type T is not derived from Asset!");
	...
	struct AssetView
	{
		AssetView(std::unordered_map<GUID, Ref<Asset>> *assets, std::vector<GUID> *data) :
			m_Assets(assets),
			m_Data(data)
		{
		}
	...
	public:
		...
		{
			static_assert(std::is_base_of_v<Asset, T>, "Get only works for types derived from Asset!");
		...
		{
			static_assert(std::is_base_of_v<Asset, T>, "Create only works for types derived from Asset!");
		...
		const std::unordered_map<GUID, Ref<Asset>>::const_iterator cbegin() const
		{
			return s_Assets.cbegin();
		}

		const std::unordered_map<GUID, Ref<Asset>>::const_iterator cend() const
		{
		...
		static std::string GetPath(const GUID &id)
		{
			if (!Exists(id))
				return "";
			return s_Assets[id]->GetPath();
		}
